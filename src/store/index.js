    ...
    login({commit}, user) {
      return new Promise((resolve, reject) => {
        login(user.account, user.password).then(data => {
          commit('SET_TOKEN', data.data.data['sessionToken'])
    ...
    getUserInfo({commit, state}) {
      ...
      return new Promise((resolve, reject) => {
        ...
        getUserInfo(getToken()).then(data => {
          if (data.data.data) {
            commit('SET_ID', data.data.data.userId)
            commit('SET_ACCOUNT', data.data.data.email)
            commit('SET_NAME', data.data.data.nickname)
    ...
    register({commit}, user) {
      return new Promise((resolve, reject) => {
        register(user.account, user.nickname, user.password).then((data) => {
          commit('SET_TOKEN', data.data.data['sessionToken'])
          // alert('test'+data.data.data.sessionToken)
          setToken(data.data.data['sessionToken'])
          resolve()
        }).catch((error) => {
          reject(error)
